...
func _ready():
	...
	_castVenom()
	$CanvasLayer/ColorRect.material.set_shader_param("softness", 6)
...
func _connectSignal():
	...
	for power in $Traps.get_children():
		if power.is_in_group("Power"):
			power.connect("playerAffected", self, "_onPlayerHurt")
...
func _onPlayerHurt():
	var currentValue = $CanvasLayer/ColorRect.material.get_shader_param("softness")
	$CanvasLayer/ColorRect.material.set_shader_param("softness", currentValue - 1)
	$IncreaseHealth.start()
